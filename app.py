...
@admin_bp.route('/unidade/cadastrar', methods=['GET', 'POST'])
@admin_required
def cadastrar_unidade():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('admin.unidade_medida'))

    return render_template('cadastrar_medida.html')

@admin_bp.route('/unidade/editar/<int:cod>', methods=['GET', 'POST'])
@admin_required
def editar_medida(cod):
    ...
    cursor.close()
    conn.close()
    return render_template('editar_medida.html', unidade_medida=unidade)
